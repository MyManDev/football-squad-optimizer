...
Minute bins still have independent player draws: no claim of a full lineup or true
substitution-hazard model. CS uses the conservative full-match clean-sheet event.
...
def sample_football_events(components: pd.DataFrame, *, samples: int, seed: int) -> FootballDraws:
    ...
    for fixture, match in frame.groupby("fixture", sort=True):
        ...
        for sample in range(samples):
            ...
            for side, group in enumerate(groups):
                ...
                for _ in range(int(goal_draws[sample, side])):
                    if gw.sum() <= 0 or rng.random() >= credit:
                        continue  # uncredited/own goal, never assigned to an absent player
                    scorer = int(rng.choice(len(group), p=gw / gw.sum()))
                    goals[scorer] += 1
                    assist_weight = aw.copy()
    ...
    return FootballDraws(pd.DataFrame(records), pd.DataFrame(scores), seed)
    ...
    *,
    ...
    for scenario, outcomes in draws.outcomes.groupby("scenario_id", sort=True):
        ...
        for name, decision in decisions.items():
            ...
            score = score_frozen_squad_decision(decision, roster).total_points - hits.get(name, 0.0)
