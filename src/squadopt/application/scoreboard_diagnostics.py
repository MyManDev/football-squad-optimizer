...
def score_recorded_decision(
    ...
    if "expected_minutes" in projected and set(playing) <= set(projected.index):
        ...
        if (
            not projected.loc[playing, "expected_minutes"]
            .map(lambda value: isinstance(value, bool))
            .any()
            and minutes.map(math.isfinite).all()
            and (minutes >= 0).all()
        ):
    ...
    if (
        ...
        and not isinstance(projected.at[captain, "expected_points"], bool)
    ):
    ...
    if vice is not None and bench is not None:
        ...
        try:
            frozen = FrozenSquadDecision(
                ...
                completion_policy=decision.get("completion_policy", "recorded_lineup"),
            )
            score = score_frozen_squad_decision(frozen, outcomes)
        except EvaluationValidationError as error:
            raise DataError(f"Frozen scoreboard decision cannot be scored: {error}") from error
