...
test("a wide screen carries this week left of the column and next week right of it", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1600, height: 900 });
  await page.goto(`/gw/${season}/1`);
  const left = page.getByRole("complementary", { name: "Bu hafta" });
...
test("a phone keeps both lists closed under the page and never scrolls sideways", async ({
  ...
  await expect(page.getByRole("complementary", { name: "Bu hafta" })).toBeHidden();
