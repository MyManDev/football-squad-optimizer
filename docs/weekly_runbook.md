...
without publication. To publish from that run, add `--publish --publish-suffix 8`, choosing
an unused suffix: GW05 then uses `feature/gw05-decision-site-8`. An existing suffixed branch
still refuses. Without a suffix the original branch name is unchanged. `--dry-run` prints
the recording choice, suffix and branch; it writes nothing. Use the same options on resume.
