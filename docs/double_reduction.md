...
Do the fitted appearance probability and the availability multiplier reduce the same risk twice?

**What this much record supports.** 2 settled week(s). One bucket (1) is thick enough to read, which gives a level and not a contrast: the question is whether the gap differs between recently absent players and present ones, and that needs two.

| Settled week | Rows | Priced below full, read | Below full, no fitted value | Earlier weeks held | Fitted source |
| --- | --- | --- | --- | --- | --- |
