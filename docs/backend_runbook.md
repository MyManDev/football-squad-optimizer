...
| job stays `running` after `advice_job_completion_deferred` | the answer was finished but the queue lock stayed busy for the 30 s completion budget. The answer is already in the cache, so asking again is served from it; recovery walks the job back once its lease runs out, and the next claim closes it from the cache (`advice_job_answer_reused`) without solving again |
