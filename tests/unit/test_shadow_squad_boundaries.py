...
Every number the squad-gate amendment froze is attacked here at its edge: the two
inclusive gate bands, the five pinned scenario knobs, the three controls the amendment
records as still unfixed, the sign and value of the frozen shift, the 30-fold sample
floor, and the two comparison conventions the tail rate rests on (which quantile rule
produces q10, and whether a realized score sitting exactly on it counts as below).
...
* S2 uses 100 folds, because its statistic is a fraction of folds: 4/100 and 16/100 are
  ...
  37-fold season neither bound is representable at all.
"""
...
def _config(**overrides: Any) -> SquadShadowConfig:
    """The three unfixed controls named, as the amendment demands, plus overrides."""

    settings: dict[str, Any] = {
        "bench_weight": 0.1,
        "decision_universe": "full_roster",
        "min_history_folds": 8,
    }
    settings.update(overrides)
    return SquadShadowConfig(**settings)
...
def test_the_named_bench_weight_reaches_the_optimizer_unchanged(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """An unfixed control is only honest if the value recorded is the value used."""

    collaborators = _KeyedCollaborators(_uniform(2, _Reading(at_or_below=100)))
    collaborators.install(monkeypatch)
    evaluate_squad_gates(
        _folds(2), _residuals(), PROVENANCE, _config(bench_weight=0.0), _zero_shift()
    )

    assert [config.bench_weight for config in collaborators.optimization_configs] == [0.0, 0.0]
...
# --------------------------------------------------------------------------------
# 3. The five pinned knobs refuse any other value.
# --------------------------------------------------------------------------------
...
def test_three_generator_knobs_are_left_to_library_defaults(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """FINDING, recorded as it stands: not every scenario knob is named.

    ``_scenario_config``'s own docstring says nothing is left to a library default, but
    ``min_player_observations``, ``player_scale_shrinkage`` and
    ``player_location_shrinkage`` are inherited from ``ScenarioConfig``. All three
    change the scenario spread, hence the PIT and the tail rate, and none of them is
    recorded in the report's provenance. This test pins the inherited values so a
    library-side change to any of them breaks loudly instead of moving a gate.
    """
    ...
    assert generated.min_player_observations == ScenarioConfig().min_player_observations
...
# --------------------------------------------------------------------------------
# 4. The three unfixed controls are required, and validated.
# --------------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ("omitted", "supplied"),
    [
        ("bench_weight", {"decision_universe": "full_roster", "min_history_folds": 8}),
        ("decision_universe", {"bench_weight": 0.1, "min_history_folds": 8}),
        ("min_history_folds", {"bench_weight": 0.1, "decision_universe": "full_roster"}),
    ],
)
def test_an_unfixed_control_has_no_default(omitted: str, supplied: dict[str, Any]) -> None:
    """A run that cannot name one of the three does not start."""

    with pytest.raises(TypeError, match=omitted):
        SquadShadowConfig(**supplied)


def test_no_unfixed_control_can_be_omitted_at_all() -> None:
    with pytest.raises(TypeError, match="bench_weight"):
        SquadShadowConfig()


@pytest.mark.parametrize(
    "bench_weight",
    [1.0000001, -1e-9, 2.0, -0.5, float("nan"), float("inf"), True, 0, 1, "0.1", None],
)
def test_an_out_of_range_bench_weight_is_refused(bench_weight: object) -> None:
    """Including the two integer endpoints: the control must be named as a float."""

    with pytest.raises(SquadShadowError, match="must be a float in"):
        _config(bench_weight=bench_weight)


@pytest.mark.parametrize("bench_weight", [0.0, 0.1, 0.5, 1.0])
def test_a_bench_weight_inside_the_unit_interval_is_accepted(bench_weight: float) -> None:
    assert _config(bench_weight=bench_weight).bench_weight == bench_weight


@pytest.mark.parametrize(
    "universe",
    ["", "FULL_ROSTER", "full-roster", "candidate pool", "starting_xi", None, 0],
)
def test_an_unknown_decision_universe_is_refused(universe: object) -> None:
    with pytest.raises(SquadShadowError, match="decision_universe must be"):
        _config(decision_universe=universe)


def test_the_implemented_decision_universe_is_accepted() -> None:
    """``full_roster`` is the universe the runner actually chooses over."""

    assert _config(decision_universe="full_roster").decision_universe == "full_roster"


def test_the_candidate_pool_decision_universe_is_refused_as_not_implemented() -> None:
    """A name the runner would not honour is refused rather than recorded.

    ``candidate_pool`` is one of the two universes the amendment names, and it is a
    spelling the range check above therefore has to admit. The squad is nonetheless
    chosen over the whole roster, so accepting the name while ignoring it would let a
    run declare a universe it did not use -- exactly the mis-declaration the unfixed
    controls exist to prevent.
    """

    with pytest.raises(SquadShadowError, match="is not implemented"):
        _config(decision_universe="candidate_pool")


@pytest.mark.parametrize(
    "min_history_folds",
    [1, 0, -1, -8, True, False, 8.0, 8.5, "8", None],
)
def test_a_min_history_folds_below_two_or_non_integer_is_refused(
    min_history_folds: object,
) -> None:
    """Booleans included: ``True`` is an int in Python and would read as one fold."""

    with pytest.raises(SquadShadowError, match="must be an integer of at least 2"):
        _config(min_history_folds=min_history_folds)


@pytest.mark.parametrize("min_history_folds", [2, 3, 8, 40])
def test_a_min_history_folds_of_two_or_more_is_accepted(min_history_folds: int) -> None:
    """Two is the generator's own floor, and the floor itself is admissible."""

    assert _config(min_history_folds=min_history_folds).min_history_folds == min_history_folds


def test_min_history_folds_of_one_is_refused_at_configuration_time() -> None:
    """The two floors agree, and the earlier of the two is the one that speaks.

    ``ScenarioConfig`` requires at least 2. Were ``SquadShadowConfig`` to accept 1, the
    mismatch would surface as a ``ScenarioConfigurationError`` raised from inside the
    first fold -- long after the panel had been loaded -- and that class is not a
    ``SquadShadowError``, so the CLI's own refusal handler would not catch it and the
    run would die mid-flight on an exception it cannot turn into a refusal. Refusing 1
    while the config is being built is what stops that: the run never starts.
    """
