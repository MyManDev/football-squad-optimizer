...
def _rows() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "contract_version": ["phase_c_component_oof_v1"] * 6,
            "model_version": ["phase-c-component-control-v1"] * 6,
            "feature_contract_version": ["phase_c_component_form_window_v1"] * 6,
            "target_contract_version": ["phase_c_component_targets_v1"] * 6,
            "dataset_contract_version": ["phase_c_component_dataset_v1"] * 6,
            "season": ["2022-23", "2022-23", "2022-23", "2023-24", "2023-24", "2023-24"],
            "target_gameweek": [2, 2, 2, 3, 3, 3],
            "player_id": ["p1", "p2", "p3", "p1", "p2", "p3"],
            "decision_timestamp_utc": [pd.NA] * 6,
            ...
            "minutes_target": [90, 30, pd.NA, 120, pd.NA, 20],
            "points_target": [7, 1, pd.NA, -1, pd.NA, 2],
            ...
            "raw_expected_points_if_appearance": [6, 5, 2.5, -1, 0, pd.NA],
            "expected_points_if_appearance": [6, 5, 2.5, 0, 0, pd.NA],
            "control_expected_points": [4.8, 2, 0.5, 0, 0, pd.NA],
        }
    )
...
def test_public_points_must_be_structurally_composed() -> None:
    rows = _rows()
    rows.loc[0, "control_expected_points"] = 4.7
...
def test_negative_raw_conditional_points_are_clipped_only_at_public_boundary() -> None:
    rows = _rows()
    rows.loc[0, "raw_expected_points_if_appearance"] = -1.0
    rows.loc[0, "expected_points_if_appearance"] = 0.0
    rows.loc[0, "control_expected_points"] = 0.0
...
def test_direct_control_fallback_does_not_invent_component_predictions() -> None:
    ...
    assert result.overall.points.observations == 3
...
def test_appearance_label_must_match_realized_minutes() -> None:
    ...
    with pytest.raises(EvaluationValidationError, match="must leave conditional"):
        evaluate_component_oof(rows)


def test_blank_gameweek_predictions_must_be_zero() -> None:
    rows = _rows()
    rows.loc[4, "appearance_target"] = 1
    rows.loc[4, "minutes_target"] = 1
