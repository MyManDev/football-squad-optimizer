...
def _report(**overrides: object) -> ShadowCalibrationReport:
    """An abstained report by default: the shape every attack below starts from."""
...
def _synthetic_squad_gates(
    ...
    config = squad.SquadShadowConfig(
        bench_weight=0.1, decision_universe="full_roster", min_history_folds=3
    )
...
def test_an_empty_declared_gate_set_cannot_claim_a_pass() -> None:
    """No completeness claim at all is the weakest possible one, and it is refused."""

    with pytest.raises(ShadowReportError, match="requires declared_gates"):
        _claims_a_pass(gate_results=_p1_sub_gates(), declared_gates=())
...
def test_only_the_squad_gates_cannot_claim_the_full_protocol() -> None:
    with pytest.raises(ShadowReportError, match="has no entry in gate_results"):
        _claims_a_pass(gate_results=_squad_gate_pair(), declared_gates=PREREG_GATES)


def test_only_p1_cannot_claim_the_full_protocol() -> None:
    with pytest.raises(ShadowReportError, match="has no entry in gate_results"):
        _claims_a_pass(gate_results=_p1_sub_gates(), declared_gates=PREREG_GATES)
...
def test_a_meaningless_declared_family_is_refused_by_the_contract() -> None:
    ...
    with pytest.raises(ShadowReportError, match="non-empty name"):
        _claims_a_pass(gate_results=(_gate(" "),), declared_gates=(" ",))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("declared_gates", {"declared_gates": ("P1_player_coverage",)}),
...
def test_the_meta_gate_alone_would_stop_only_the_probability_named_diagnostics(
    ...
    catch the two probability-named squad diagnostics and let the three bookkeeping ones
    through. They are not probabilities, so this is a limit of the pattern rather than a
    hole in the boundary — but the pattern is not the second line of defence it reads as.
    """
