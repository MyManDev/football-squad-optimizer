...
def _config(**overrides: object) -> SquadShadowConfig:
    ...
    return SquadShadowConfig(**settings)  # type: ignore[arg-type]
...
def test_the_squad_instrument_reports_no_interval_although_it_claims_one(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """DEFECT (documentation): the promised fold-level bootstrap is not run here.

    The module docstring says the interval comes from "the existing fold-level
    bootstrap", and the pre-registration fixes it at 5000 resamples, 90%, seed 0.
    ``bootstrap_interval`` is never imported or called, the diagnostics carry no
    interval, and the runner passes ``interval_diagnostics={}``, so a passing
    squad-gate report states two rates with no uncertainty at all.
    """

    folds, readings = _population(0.50, tail_count=5)
    _install(monkeypatch, readings=readings)
    _, _, diagnostics = evaluate_squad_gates(
        folds, _development_residuals(), _provenance(), _config(), _shift()
    )
    ...
    assert not hasattr(squad_module, "bootstrap_interval")
    assert report_to_dict(_combine())["interval_diagnostics"] == {}  # type: ignore[arg-type]
...
def test_an_unimplemented_decision_universe_is_refused_rather_than_declared_and_ignored(
    ...
    Both universes are supposed to give different squads and different PIT, which is
    why the amendment records the choice as unfixed. Only one of them is built: the
    squad is always optimized over the whole projection frame. Accepting
    ``candidate_pool`` as a name while measuring the full roster would put a false
    control into the artifact, so the configuration refuses it outright, and the one
    universe that is implemented is the one the readings come from.
    """
...
def test_the_three_unfixed_controls_have_no_defaults() -> None:
    with pytest.raises(TypeError):
        SquadShadowConfig()  # type: ignore[call-arg]
    with pytest.raises(SquadShadowError, match="bench_weight must be a float"):
        _config(bench_weight=1)
    with pytest.raises(SquadShadowError, match="decision_universe must be"):
        _config(decision_universe="whatever_pool")
    with pytest.raises(SquadShadowError, match="min_history_folds must be"):
        _config(min_history_folds=0)
...
def test_min_history_folds_of_one_is_refused_at_configuration_time() -> None:
    """The accepted range agrees with the contract that consumes it.

    ``ScenarioConfig`` requires at least two history folds. A run declared with
    ``--min-history-folds 1`` would otherwise start, load the panel, read the residual
    table, and only then die inside its first fold with a
    ``ScenarioConfigurationError`` — the expensive way to learn a number is illegal.
    ``SquadShadowConfig`` now carries the same floor, so the refusal happens before
    anything is read; the generator's floor is asserted alongside it so the two cannot
    drift apart.
    """

    with pytest.raises(SquadShadowError, match="at least 2"):
    ...
    # Two is the floor on both sides, and it is accepted on both.
    assert _config(min_history_folds=2).min_history_folds == 2
    assert ScenarioConfig(min_history_folds=2).min_history_folds == 2
...
def test_the_scenario_config_pins_every_knob_the_run_declares(
    ...
    assert scenario_config.min_history_folds == 2
