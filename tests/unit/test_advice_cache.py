...
def test_a_move_that_another_move_overtook_leaves_the_file_to_it(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events = _recorded_events(monkeypatch)
    path = tmp_path / "ab" / f"{'ab' * 32}.json"
    path.parent.mkdir(parents=True)
    path.write_bytes(b"")
    later = path.with_name(f"{path.name}.damaged-later")

    def overtaken(source: Path, destination: Path) -> None:
        # Windows: both renames report success, and the file ends at the one that landed last.
        os.replace(source, destination)
        os.replace(destination, later)

    monkeypatch.setattr(_damaged_entry, "replace_retrying", overtaken)
    _damaged_entry.quarantine(path, b"", component="cache")  # used to raise FileNotFoundError
    assert [p.name for p in path.parent.iterdir()] == [later.name]
    assert events == []  # the caller holding the file logs it


def test_a_whole_entry_a_later_move_took_is_left_to_that_move(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events = _recorded_events(monkeypatch)
    path = tmp_path / "ab" / f"{'ab' * 32}.json"
    path.parent.mkdir(parents=True)
    path.write_bytes(b'{"answer": 1}')  # a writer published before this reader's move
    later = path.with_name(f"{path.name}.damaged-later")
    link = os.link

    def taken_first(source: Path, destination: Path) -> None:
        os.replace(source, later)
        link(source, destination)
...
def test_a_read_refused_while_a_move_is_in_flight_is_retried(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    cache = FileAdviceCache(tmp_path)
    key = _key()
    ...
    refusals = [PermissionError(13, "in use"), PermissionError(13, "in use")]

    def refused_twice(self: Path) -> bytes:
        if refusals:
            raise refusals.pop()
        return read_bytes(self)

    monkeypatch.setattr(Path, "read_bytes", refused_twice)
    assert cache.get(key) == b'{"answer": 1}'  # used to raise PermissionError out of get
    assert refusals == []


def test_a_read_refused_for_longer_than_a_move_is_raised(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    cache = FileAdviceCache(tmp_path)
    key = _key()
    ...
    def always_refused(self: Path) -> bytes:
        attempts.append(self)
        raise PermissionError(13, "held")

    monkeypatch.setattr(Path, "read_bytes", always_refused)
    monkeypatch.setattr(_damaged_entry.time, "sleep", lambda _seconds: None)
    with pytest.raises(PermissionError):
        cache.get(key)
    assert len(attempts) == _damaged_entry.SETTLE_ATTEMPTS


def test_a_winner_moved_aside_for_a_moment_is_linked_over_not_an_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    cache = FileAdviceCache(tmp_path)
    key = _key()
    ...
    def racing_get(inner_key: str) -> bytes | None:
        calls.append(inner_key)
        if len(calls) == 1:  # the writer's miss; another writer publishes right after it
            missed = get(inner_key)
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(answer)
            return missed
        if len(calls) == 2:  # a reader that read damage earlier moves the winner aside
            os.replace(path, path.with_name(f"{path.name}.damaged-reader"))
        return get(inner_key)

    monkeypatch.setattr(cache, "get", racing_get)
    cache.put(key, answer)  # used to raise "exists but cannot be read back"
    assert path.read_bytes() == answer
    assert len(calls) == 2


def _race_over_a_torn_entry(
    root: Path, events: list[tuple[str, dict[str, object]]]
) -> tuple[list[str], bytes | None, list[bytes], int]:
    """Four readers and one writer, released together onto a key holding b""."""
    ...
    held = path.read_bytes() if path.exists() else None
    asides = [p.read_bytes() for p in path.parent.iterdir() if ".damaged-" in p.name]
    return errors, held, asides, len(events)


def test_readers_and_a_writer_racing_over_a_torn_entry_leave_the_answer(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """On Windows two moves of one file can both report success; nothing may escape.

    Where renames go by name (Linux, macOS) the later move finds the file gone, and this
    passes without exercising the Windows case.
    """
    ...
    for index in range(100):
        events.clear()
        outcome = _race_over_a_torn_entry(tmp_path / str(index), events)
        if outcome != ([], answer, [b""], 1):
            wrong.append(f"round {index}: {outcome!r}")
    assert wrong == [], f"{len(wrong)} of 100 rounds went wrong; first: {wrong[:3]}"
