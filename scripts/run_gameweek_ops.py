...
This is the opening-week runbook as a machine. **decide** (before the deadline) reads
a capture, builds the operational-control recommendation, verifies every runbook
check as code — control model identity, availability applied, squad structure,
budget, club limit, captain sanity, risk not silently attached — and only then
freezes the decision into the season ledger. A failed check exits 1 and records
nothing: an unverified decision is not published.
...
def verify_decision(recommendation: Recommendation, projection: Projection) -> list[str]:
    ...
    if recommendation.total_cost_tenths > settings.budget_tenths:
        failures.append(
...
def _decide(arguments: argparse.Namespace) -> int:
    ...
    failures = verify_decision(recommendation, projection)
    if failures:
