...
def _chain_record(result: SeasonChainResult, label: str, elapsed: float) -> dict[str, object]:
...
def _paired_weekly_differences(
    ...
    base_by_week = {
        ...
        for week in _rows(baseline["weeks"])
    }
    return [
        (
        ...
        for week in _rows(candidate["weeks"])
...
def _comparison(
    ...
    *,
    ...
    for candidate in chains:
        ...
        if baseline is None:
            continue
        pairs.extend(_paired_weekly_differences(candidate, baseline))
...
def _merge(arguments: argparse.Namespace) -> int:
    ...
    for doc in documents:
        chains.extend(_rows(doc["chains"]))
