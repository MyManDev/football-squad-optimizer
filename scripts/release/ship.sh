...
case "$#" in 5|6) ;; *) echo 'usage: ship.sh [--dry-run] <site PR> <tag> <release branch> <accepted generated-at ISO> <summary> [settled gameweek]' >&2; exit 2;; esac
SITE_PR="$1"; TAG="$2"; BR="$3"; ACCEPTED_STAMP="$4"; SUMMARY="$5"; SETTLED="${6:-}"
...
Run the live smoke checks and content checks matching accepted stamp $ACCEPTED_STAMP${SETTLED:+, settling gameweek $SETTLED}; retry once.
EOF
  exit 0
fi
...
"$PY" "$S/verify_live.py" "$ACCEPTED_STAMP" $SETTLED_ARGS || { sleep 60; "$PY" "$S/verify_live.py" "$ACCEPTED_STAMP" $SETTLED_ARGS || { log "live verification found failures"; exit 1; }; }
log "LIVE"
