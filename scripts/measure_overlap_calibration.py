"""Is the overlap-scaled edge a calibrated claim in all nine cells? The gate, applied.

    python -m scripts.measure_overlap_calibration
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--json-output",
        type=Path,
        default=REPOSITORY_ROOT / "docs" / "overlap_calibration.json",
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        default=REPOSITORY_ROOT / "docs" / "overlap_calibration.md",
    )
...
def main() -> int:
                    ...
                    rows.append(
                        {
                            ...
                            "claimed_unscaled": float(unscaled.probability_ahead),
                            "edge_scale": float(str(claim.diagnostics["edge_scale"])),
    ...
    for horizon in horizons:
        ...
        for family in FAMILIES:
            ...
            claimed_unscaled = np.asarray([c["claimed_unscaled"] for c in cells], dtype="float64")
            ...
            gap_unscaled = float(claimed_unscaled.mean() - realized_flags.mean())
            ...
            by_cell[f"h{horizon}:{family}"] = {
                ...
                "gap": gap,
                "gap_unscaled": gap_unscaled,
                "gap_worsening_vs_anchored": abs(gap) - abs(gap_unscaled),
    ...
    protected_ok = True
    for cell_key, cell_values in by_cell.items():
        cell_family = cell_key.split(":", 1)[1]
        if cell_family in ("risk_neutral", "contrarian"):
            worsening = float(str(cell_values["gap_worsening_vs_anchored"]))
            if worsening > 0.03:
                protected_ok = False
    verdict = {
        "passes_protected_families": bool(protected_ok),
        ...
        "passes": bool(passes_identity and passes_calibration and ordering_ok and protected_ok),
        "note": (
            "Clauses of overlap_scaled_edge_prereg.md as declared; a pass here "
    ...
    document = {
        **artifact_metadata(panel_rows=len(panel), created_utc=created_utc),
        "contract_version": "overlap_calibration_v1",
        "config": {
            "horizons": horizons,
            "scenario_count": int(arguments.scenario_count),
    ...
    lines += [
        ...
        f"protected {'ok' if protected_ok else 'FAILED'}, "
